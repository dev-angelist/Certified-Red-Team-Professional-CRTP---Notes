...
To dump credentials from **LSASS** on a local machine using **Invoke-Mimikatz**, run:
...
OverPass-The-Hash (OPTH) that allows attackers to generate authentication tokens from NTLM hashes or Kerberos keys.

In the case of Pass-The-Hash it's necessary to read NTLM hash and use it to autheticate 'as a password', while OverPass-The-Hash uses NTLM or AES Key to obtain TGT Kerberos and authenticate on the system (usually spawning a new powershell session).
...
DCSync allows an attacker to extract password hashes from a Domain Controller **without executing code on it**. This is a stealthier approach compared to direct LSASS dumping.
